// Vercel serverless function for AI design generation with custom prompts
// This endpoint uses YOUR Gemini API key to generate designs based on user's custom text description
...
module.exports = async function handler(req, res) {
  ...
  try {
    console.log('=== GENERATE DESIGN (CUSTOM PROMPT) REQUEST START ===');
    ...
    if (!user) {
      ...
      await trackApiUsage({
        userId: null,
        email: null,
        endpoint: '/api/generate-design-custom',
        method: req.method,
        statusCode: 401,
    ...
    // Parse request
    const { image, customPrompt, model = 'gemini-2.5-flash-image' } = req.body;
    // Always use the correct model, ignore what client sends
    // const model = 'gemini-2.5-flash-image';
    console.log('Request params:', {
      hasImage: !!image,
      imageLength: image ? image.length : 0,
      customPrompt: customPrompt ? customPrompt.substring(0, 100) + '...' : 'missing',
      model
    });

    if (!image || !customPrompt) {
      console.log('Missing required parameters:', { image: !!image, customPrompt: !!customPrompt });
      return res.status(400).json({ message: 'Missing required parameters: image and customPrompt' });
    }
    ...
    // Use the custom prompt directly - it already has enhancement in the client
    console.log('Using custom prompt:', customPrompt);

    // Edit/transform the user's room image using Gemini 2.5 Flash
    console.log('Transforming image with Gemini 2.5 Flash (custom prompt)...');
    ...
    await logGeneration({
      ...
      customPrompt: customPrompt.substring(0, 200), // Log first 200 chars
      processingTime,
      creditsUsed,
    ...
    console.error('=== CUSTOM DESIGN GENERATION ERROR ===');
    ...
    if (error.message.includes('safety') || error.message.includes('blocked')) {
      statusCode = 400;
      errorResponse = {
        message: 'Content blocked by safety filters. Please try a different description.',
        debug: error.message
      };
    } else {
      statusCode = 500;
